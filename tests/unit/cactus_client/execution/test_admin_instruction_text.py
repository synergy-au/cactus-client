...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [instr("ensure-end-device", registered=True, has_der_list=True)],
            "Register EndDevice, with DER list",
        ),
        (
            [instr("ensure-end-device", registered=False)],
            "Remove EndDevice registration",
        ),
        (
            [instr("ensure-end-device", registered=True, client="dev1")],
            "Register EndDevice for dev1",
        ),
        ...
        (
            [instr("ensure-fsa", annotation="fsa1")],
            "Ensure FunctionSetAssignment'fsa1'",
        ),
        (
            [instr("ensure-fsa", annotation="fsa1", primacy=1)],
            "Ensure FunctionSetAssignment'fsa1' primacy=1",
        ),
        ([instr("ensure-fsa", primacy=0)], "Ensure FunctionSetAssignment primacy=0"),
        # ensure-der-program
        ([instr("ensure-der-program")], "Ensure DERProgram"),
        (
            [instr("ensure-der-program", fsa_annotation="fsa1")],
            "Ensure DERProgram'fsa1'",
        ),
        (
            [instr("ensure-der-program", fsa_annotation="fsa1", primacy=2)],
            "Ensure DERProgram'fsa1' primacy=2",
        ),
        # set-client-access
        ([instr("set-client-access", granted=True)], "Grant client access"),
        ([instr("set-client-access", granted=False)], "Revoke client access"),
        (
            [instr("set-client-access", granted=True, client="dev2")],
            "Grant client access for dev2",
        ),
        # ensure-der-control-list
        ([instr("ensure-der-control-list")], "Ensure DERControlList accessible"),
        (
            [instr("ensure-der-control-list", subscribable=True)],
            "Ensure DERControlList accessible, subscribable",
        ),
        # create-der-control
        (
        ...
        (
            [instr("create-default-der-control", opModExpLimW=200.0)],
            "Create DefaultDERControl opModExpLimW=200.0",
        ),
        # clear-der-controls
        ...
        (
            [
                instr("ensure-mup-list-empty"),
                instr("ensure-end-device", registered=True),
            ],
